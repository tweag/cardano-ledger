...
optsParser = hsubparser
  ...
  <> command "tee" (info teeCommand (progDesc "Display contents of canonical ledger state file and store it to the new one"))
  )
  where
    ...
    teeCommand = Tee <$> argument str (metavar "SCLS_FILE")
                     <*> argument str (metavar "SCLS_FILE")
        ...
        withFile oldFile ReadMode $ \hdl -> do
          External.serialize
            ...
            (defaultSerializationPlan
               & (let p = (Proxy @"utxo/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
               & (let p = (Proxy @"blocks/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
               & (let p = (Proxy @"gov/committee/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
               & (let p = (Proxy @"gov/constitution/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
               & (let p = (Proxy @"gov/pparams/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
               & (let p = (Proxy @"gov/proposals/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
               & (let p = (Proxy @"pool_stake/v0") in addNamespacedChunks p (knownNamespacedData hdl p & S.duplicate & S.print))
